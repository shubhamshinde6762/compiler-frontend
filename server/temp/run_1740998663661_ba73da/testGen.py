...
def generate_testcases():
    ...
    for i in range(T):
        ...
        arr = sorted(random.randint(1, 10**3) for _ in range(n))  # Sorted array
        
        if i % 2 == 0:  # Ensure True cases
            target = arr[n // 2]  # Choose a middle element
            majority_count = (n // 2) + 2  # Ensure it's a majority element
            arr = sorted(arr[:majority_count] + [target] * (2 * majority_count) + arr[majority_count:])
        else:
        ...
        testcases.append((len(arr), arr, target))
