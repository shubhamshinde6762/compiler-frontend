...
def print_test_case(tc):
    ...
    print(len(tc["calls"]))
    # Print the array of call objects.
    print("[")
    for call in tc["calls"]:
        print(f'  {{ "key": "{call["key"]}", "time": {call["time"]} }},')
    print("]")
...
def generate_all_test_cases():
    ...
    # Append edge/tricky cases manually if needed.
    # For instance, a test case with 0 calls.
    test_cases.append({
    ...
    # A tricky case: calls with the same time.
    test_cases.append({
    ...
    # A tricky case: keys with minimum and maximum lengths.
    test_cases.append({
