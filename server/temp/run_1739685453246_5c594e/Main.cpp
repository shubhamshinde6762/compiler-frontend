...
int main(){
    ...
    while(T--){
        ...
        int n;
        cin >> n;
        n++; // Input n is given as (number of nodes - 1); adjust to actual node count.
        int m;
        cin >> m; // m is always 2 (columns) per the input format.
        ...
        // Compute distances from any source using BFS.
        ...
        // Compute farthest node and distances from a source.
        ...
        int vA = p1.first;
        auto p2 = bfsFarthest(vA);
        int vB = p2.first;
        vector<int> dA = p2.second;
        vector<int> dB = bfsAll(vB);
 
        // Compute parent and depth using BFS (to avoid recursion and stack overflow)
        ...
        // Validate: For each node i, the candidate answer must be at maximum distance from i.
        bool valid = true;
        for (int i = 0; i < n; i++){
