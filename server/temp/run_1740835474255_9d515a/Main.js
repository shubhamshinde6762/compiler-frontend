class Solution {
    highFive(items) {
        ...
        // Store scores in a Map with student ID as key
        ...
        for (let [id, scoreList] of [...scores.entries()].sort((a, b) => a[0] - b[0])) {
            scoreList.sort((a, b) => b - a); // Sort in descending order
            let avg = Math.floor(scoreList.slice(0, 5).reduce((sum, score) => sum + score, 0) / 5);
            result.push([id, avg]);
        }
